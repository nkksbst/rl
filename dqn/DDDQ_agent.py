...
class DuelingDDQAgent():
  ...
  # update Q values here
    ...
    q_next = T.sum(V_next, A_next - A_next.mean(dim=1, keepdim= True))
    
    V_, A_ = self.q_eval.forward(states_)    
    q_eval = T.add(V_, A_ - A_.mean(dim=1, keepdim= True))
    max_actions = T.argmax(q_eval)
